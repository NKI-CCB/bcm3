...
load_bcm3output <- function(model_name, output_folder, prior_file="prior.xml", output_filename="output.nc", group_name="") {
  model <- list()
  model$name = model_name
  model$output_folder <- paste(model_name, "/", output_folder, sep="")
  
  # Prior
  model$prior <- list()
  varxml <- xmlTreeParse(paste(model_name, "/", prior_file, sep=""))
...
as.coda.mcmc <- function(model) {
  require(coda)
  return(mcmc(model$samples[,1:model$nvar]))
}

as.ggmcmc <- function(model) {
  require(coda)
  require(ggmcmc)
  return(ggs(mcmc(model$samples[,1:model$nvar])))
}
