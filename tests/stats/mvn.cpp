...
BOOST_AUTO_TEST_CASE(dmvnorm_1d)
{
    ...
    BOOST_CHECK_CLOSE(bcm3::dmvnormal(-VectorReal::Ones(1), mu, sigma), bcm3::PdfNormal(-1, mu(0), sigma(0, 0)), 1e-13);
	BOOST_CHECK_CLOSE(bcm3::dmvnormal(VectorReal::Zero(1), mu, sigma, true), bcm3::LogPdfNormal(0, mu(0), sigma(0, 0)), 1e-13);
	BOOST_CHECK_CLOSE(bcm3::dmvnormal(VectorReal::Ones(1), mu, sigma, true), bcm3::LogPdfNormal(1, mu(0), sigma(0, 0)), 1e-13);
	BOOST_CHECK_CLOSE(bcm3::dmvnormal(-VectorReal::Ones(1), mu, sigma, true), bcm3::LogPdfNormal(-1, mu(0), sigma(0, 0)), 1e-13);
}
...
BOOST_AUTO_TEST_CASE(dmvnorm_2d_identity)
{
    ...
    BOOST_CHECK_CLOSE(bcm3::dmvnormal(-VectorReal::Ones(2), mu, sigma), 0.05854983152431917, 1e-12);
	BOOST_CHECK_CLOSE(bcm3::dmvnormal(VectorReal::Zero(2), mu, sigma, true), -1.837877066409345, 1e-12);
	BOOST_CHECK_CLOSE(bcm3::dmvnormal(VectorReal::Ones(2), mu, sigma, true), -2.837877066409345, 1e-12);
	BOOST_CHECK_CLOSE(bcm3::dmvnormal(-VectorReal::Ones(2), mu, sigma, true), -2.837877066409345, 1e-12);
}
...
BOOST_AUTO_TEST_CASE(dmvnorm_2d)
{
	...
	BOOST_CHECK_CLOSE(bcm3::dmvnormal(mu, mu, sigma, true), -7.894197416778156, 1e-12);

    VectorReal x(2);
    x(0) = 1000.0;
    x(1) = 1050.0;
    BOOST_CHECK_CLOSE(bcm3::dmvnormal(x, mu, sigma), 6.617956105689106e-45, 1e-12);
	BOOST_CHECK_CLOSE(bcm3::dmvnormal(x, mu, sigma, true), -101.726542607819, 1e-12);
}
