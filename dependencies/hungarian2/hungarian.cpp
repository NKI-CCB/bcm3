...
static const Real oo = std::numeric_limits<Real>::max();
...
struct LeftEdge {
    ...
    LeftEdge() : right(), cost() {}
    LeftEdge(int right, Real cost) : right(right), cost(cost) {}
...
__declspec(noinline) const std::vector<int> hungarianMinimumWeightPerfectMatching(const int n, const std::vector<WeightedBipartiteEdge> allEdges) {
    ...
    std::vector<Real> leftPotential(n);
    ...
    fo(i, n) {
        ...
        Real smallestEdgeCost = edges[0].cost;
    ...
    std::vector<int> leftMatchedTo(n, UNMATCHED);
    std::vector<int> rightMatchedTo(n, UNMATCHED);
    ...
    std::vector<Real> rightMinimumSlack(n);
    std::vector<int> rightMinimumSlackLeftNode(n);
    std::vector<int> rightMinimumSlackEdgeIndex(n);
    ...
    while (currentMatchingCardinality < n) {
        ...
        std::fill(rightMinimumSlack.begin(), rightMinimumSlack.end(), oo);
        std::fill(rightMinimumSlackLeftNode.begin(), rightMinimumSlackLeftNode.end(), UNMATCHED);
        ...
        while (endingRightNode == UNMATCHED) {
            ...
            while (endingRightNode == UNMATCHED && !leftNodeQueue.empty()) {
                ...
                if (endingRightNode == UNMATCHED) {
                    const Real potential = leftPotential[i];
