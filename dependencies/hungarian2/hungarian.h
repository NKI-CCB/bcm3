...
struct WeightedBipartiteEdge {
    ...
    WeightedBipartiteEdge(int left, int right, Real cost) : left(left), right(right), cost(cost) {}
};
...
const std::vector<int> hungarianMinimumWeightPerfectMatching(int n, int n_right, const std::vector<WeightedBipartiteEdge>& allEdges, int edge_count);
