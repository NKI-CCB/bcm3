...
static std::shared_ptr<LikelihoodPharmacokineticTrajectory> GetLikelihood(bcm3info* info, int* retval)
{
	if (!info) {
...
void bcm3_rbridge_PK_get_log_likelihood(char** bcm3info_ptr, double* param_values, double* logl, int* retval)
{
	...
	std::shared_ptr<LikelihoodPharmacokineticTrajectory> ll = GetLikelihood(info, retval);
	if (!info || !ll) {
		return;
	}
