			...
			for (size_t ti = 0; ti < simulation_timepoints.size(); ti++) {
				...
				if (st.species_ix != std::numeric_limits<size_t>::max() && (int)st.synchronize == synchronize_i) {
					...
					for (size_t i = 0; i < active_cells; i++) {
						...
						if (x == x) {
							data_likelihoods[st.data_likelihood_ix]->NotifySimulatedValue(st.time_ix, x, st.species_ix, i, population_size, completed_population_size, 0, cells[i]->CellCompleted());
						}
					}
				}
			}
...
bool Experiment::Load(const boost::property_tree::ptree& xml_node, const boost::program_options::variables_map& vm)
{
	...
	output_trajectories_timepoints.resize(100);
	for (size_t i = 0; i < 100; i++) {
		output_trajectories_timepoints(i) = simulation_begin_time + (simulation_end_time - simulation_begin_time) * i / (Real)(100 - 1);
	}
	simulated_trajectories.resize(max_number_of_cells);
	for (size_t i = 0; i < simulated_trajectories.size(); i++) {
		simulated_trajectories[i].resize(100, VectorReal::Constant(cell_model.GetNumSimulatedSpecies(), std::numeric_limits<Real>::quiet_NaN()));
	}
...
bool Experiment::GenerateAndCompileSolverCode(const std::string& codegen_name)
{
	...
	if (f.is_open()) {
		...
		f << std::endl;
		f << "inline double square(double x) { return x * x; }\n";
		f << std::endl;
		f << "inline double hill_function(double x, double k, double n)\n";
		f << "{\n";
		f << "\tif (x < 0.0) return -hill_function(-x, k, n);\n";
		f << "\tif (x <= DBL_EPSILON) return 0.0;\n";
		f << "\tif (x == INFINITY) return 1.0;\n";
		f << "\tdouble xn = pow(x, n);\n";
		f << "\tif (xn <= DBL_EPSILON) return 0.0;\n";
		f << "\tif (xn == INFINITY) return 1.0;\n";
		f << "\tdouble kn = pow(k, n);\n";
		...
		f << "\tif (x == INFINITY) return 1.0;\n";
		f << "\tdouble x2 = x * x;\n";
		f << "\tif (x2 <= DBL_EPSILON) return 0.0;\n";
		f << "\tif (x2 == INFINITY) return 1.0;\n";
		f << "\tdouble k2 = k * k;\n";
		f << "\tif (k2 == INFINITY) return 0.0;\n";
		f << "\treturn x2 / (k2 + x2);\n";
		f << "}\n";
		f << "inline double hill_function_fixedn4(double x, double k)\n";
		f << "{\n";
		...
		f << "\tdouble x2 = x * x;\n";
		f << "\tdouble x4 = x2 * x2;\n";
		f << "\tif (x4 <= DBL_EPSILON) return 0.0;\n";
		f << "\tif (x4 == INFINITY) return 1.0;\n";
		f << "\tdouble k2 = k * k;\n";
		f << "\tdouble k4 = k2 * k2;\n";
		f << "\tif (k4 == INFINITY) return 0.0;\n";
		f << "\treturn x4 / (k4 + x4);\n";
		f << "}\n";
		f << "inline double hill_function_fixedn10(double x, double k)\n";
		f << "{\n";
		...
		f << "\tdouble x8 = x4 * x4;\n";
		f << "\tdouble x10 = x2 * x8;\n";
		f << "\tif (x10 <= DBL_EPSILON) return 0.0;\n";
		f << "\tif (x10 == INFINITY) return 1.0;\n";
		f << "\tdouble k2 = k * k;\n";
		f << "\tdouble k4 = k2 * k2;\n";
		f << "\tdouble k8 = k4 * k4;\n";
		f << "\tdouble k10 = k2 * k8;\n";
		f << "\tif (k10 == INFINITY) return 0.0;\n";
		f << "\treturn x10 / (k10 + x10);\n";
		f << "}\n";
		f << "inline double hill_function_fixedn16(double x, double k)\n";
		f << "{\n";
		f << "\tif (x < 0.0) return -hill_function_fixedn10(-x, k);\n";
		f << "\tif (x <= DBL_EPSILON) return 0.0;\n";
		f << "\tif (x == INFINITY) return 1.0;\n";
		...
		f << "\tdouble x8 = x4 * x4;\n";
		f << "\tdouble x16 = x8 * x8;\n";
		f << "\tif (x16 <= DBL_EPSILON) return 0.0;\n";
		f << "\tif (x16 == INFINITY) return 1.0;\n";
		f << "\tdouble k2 = k * k;\n";
		f << "\tdouble k4 = k2 * k2;\n";
		f << "\tdouble k8 = k4 * k4;\n";
		f << "\tdouble k16 = k8 * k8;\n";
		f << "\tif (k16 == INFINITY) return 0.0;\n";
		f << "\treturn x16 / (k16 + x16);\n";
		f << "}\n";
		f << "inline double hill_function_fixedn100(double x, double k)\n";
		f << "{\n";
		...
		f << "\tdouble x8 = x4 * x4;\n";
		f << "\tdouble x16 = x8 * x8;\n";
		f << "\tdouble x32 = x16 * x16;\n";
		f << "\tdouble x64 = x32 * x32;\n";
		f << "\tdouble x100 = x64 * x32 * x4;\n";
		f << "\tif (x100 <= DBL_EPSILON) return 0.0;\n";
		f << "\tif (x100 == INFINITY) return 1.0;\n";
		f << "\tdouble k2 = k * k;\n";
		f << "\tdouble k4 = k2 * k2;\n";
		f << "\tdouble k8 = k4 * k4;\n";
		f << "\tdouble k16 = k8 * k8;\n";
		f << "\tdouble k32 = k16 * k16;\n";
		f << "\tdouble k64 = k32 * k32;\n";
		f << "\tdouble k100 = k64 * k32 * k4;\n";
		f << "\tif (k100 == INFINITY) return 0.0;\n";
		f << "\treturn x100 / (k100 + x100);\n";
		f << "}\n";
		f << "inline double hill_function_derivative(double x, double k, double n)\n";
		f << "{\n";
		...
		f << "\tif (s < DBL_EPSILON) return 0.0;\n";
		f << "\treturn kcat * e * s / (KM + s);\n";
		f << "}\n";
		f << "inline double michaelis_menten_derivative_enzyme(double kcat, double KM, double e, double s)\n";
		f << "{\n";
		f << "\tif (s < 0) return -kcat * s / (-KM + s);\n";
		f << "\tif (s < DBL_EPSILON) return 0.0;\n";
		f << "\treturn kcat * s / (KM + s);\n";
		f << "}\n";
		f << "inline double michaelis_menten_derivative_substrate(double kcat, double KM, double e, double s)\n";
		f << "{\n";
		f << "\tif (s < 0) return e * kcat * KM / (square(-KM + s));\n";
		f << "\tif (s < DBL_EPSILON) return 0.0;\n";
		f << "\treturn e * kcat * KM / (square(KM + s));\n";
		f << "}\n";
		f << "inline double safepow(double x, double n)\n";
		f << "{\n";
		f << "\tif (x <= 0) {\n";
		f << "\t\treturn 0.0;\n";
		f << "\t} else {\n";
